...
const TaskAllocation = () => {
  ...
  const handleAllocation = async (e) => {
    e.preventDefault();
    if (!selectedAsha) return toast.error(t('tasks.error_select_asha'));

    // Add specific validation for Village Type
    if (allocationType === "village" && !village.trim()) {
      return toast.error(t('tasks.error_enter_village'));
    }
...
try {
  ...
  const errorMessage = err.response?.data?.message || err.message || t('tasks.error_failed');
  ...
  return (
    <div className="p-6 max-w-6xl mx-auto">
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-gray-800 tracking-tight">{t('tasks.title')}</h1>
        <p className="text-gray-500 text-sm">{t('tasks.desc')}</p>
      </div>
          ...
          <div className="space-y-2 overflow-y-auto flex-1 pr-2 custom-scrollbar">
            {ashas.length > 0 ? (
                  ...
                  <div className="flex justify-between items-start">
                    <div>
                      ...
                      <div className="flex items-center gap-1 text-[11px] text-gray-500 mt-1.5 uppercase font-medium tracking-wide">
                        <MapPin size={10} className="text-emerald-500" /> {asha.village || t('tasks.no_village')}
                      </div>
                    </div>
                    {selectedAsha === asha.id && (
              ...
              <div className="flex flex-col items-center justify-center py-20 text-gray-400">
                ...
                <p className="text-xs italic">{t('tasks.no_workers')}</p>
              </div>
            )}
          </div>
        </div>
            ...
            <form onSubmit={handleAllocation} className="p-8">
              {allocationType === 'village' ? (
                <div className="space-y-4">
                  <label className="text-sm font-medium text-gray-700">{t('tasks.label_village_assign')}</label>
                  <input 
                    ...
                    placeholder={t('tasks.placeholder_village')}
                    value={village}
                ...
                <div className="space-y-4">
                  <label className="text-sm font-medium text-gray-700">{t('tasks.label_limit_assign')}</label>
                  <input 
                    type="number"
                    className="w-full p-4 border border-gray-200 rounded-xl bg-gray-50 focus:bg-white outline-none focus:ring-2 focus:ring-emerald-500 transition-all"
                    value={limit}
              ...
              <button 
                ...
                disabled={loading}
              >
                {loading ? t('tasks.loading_alloc') : t('tasks.run_allocation')} <ArrowRight size={18}/>
              </button>
            </form>
          </div>
          ...
          <div className="bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden">
            <div className="p-4 bg-gray-50/80 border-b flex justify-between items-center">
              <h3 className="text-sm font-bold text-gray-700 flex items-center gap-2">
                <ClipboardList size={16} className="text-emerald-600" /> {t('tasks.log_title')}
              </h3>
            </div>
            <div className="max-h-[300px] overflow-y-auto">
              <table className="w-full text-sm text-left">
                <thead className="sticky top-0 bg-white shadow-sm text-gray-400 text-[10px] uppercase font-bold">
                  <tr>
                    <th className="px-6 py-3">{t('tasks.table_asha')}</th>
                    <th className="px-6 py-3">{t('tasks.table_ben')}</th>
                    <th className="px-6 py-3">{t('tasks.table_village')}</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-gray-50">
                  {assignments.length > 0 ? assignments.map((asgn, idx) => (
                    ...
                    <tr>
                       <td colSpan="3" className="px-6 py-10 text-center text-gray-400 italic">{t('tasks.no_logs')}</td>
                    </tr>
                  )}
                </tbody>
              </table>
