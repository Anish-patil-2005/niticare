...
import { 
  ...
  UserCheck, MapPin, Users, ClipboardCheck, CheckSquare, Square, Loader2, Trash2
} from 'lucide-react';
...
const BeneficiaryDirectory = ({ mode = 'admin' }) => {
  ...
  const handleEditClick = (user) => {
    ...
    setEditData({
      ...
      age: user.age || '',
      health_status: user.health_status || 'normal'
    });
  };
  ...
  const handleUpdate = async (e) => {
    ...
    try {
      ...
      const response = isAshaMode 
        ? await ashaService.updateBeneficiary(selectedUser.id, payload)
      ...
      const errorMsg = err.response?.data?.message || "Check backend routes";
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto space-y-6 pb-24">
      ...
      {/* 1. Summary Stats Bar */}
      <div className={`grid grid-cols-1 ${isAshaMode ? 'md:grid-cols-2' : 'md:grid-cols-3'} gap-4`}>
        <div className="bg-white p-4 rounded-2xl border border-gray-100 shadow-sm flex items-center gap-4 cursor-pointer hover:border-emerald-200" onClick={() => setFilter('all')}>
          ...
          <div>
            <p className="text-[10px] font-bold text-gray-400 uppercase">{isAshaMode ? 'My Cases' : 'Total Tracked'}</p>
      ...
      <div className="bg-white p-6 rounded-2xl border border-gray-200 shadow-sm space-y-4">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          <h1 className="text-2xl font-bold text-gray-800">{isAshaMode ? 'My Field Directory' : 'Directory'}</h1>
        ...
        <div className="relative w-full">
          ...
          <input type="text" placeholder="Search by name or Government ID..." className="w-full pl-10 pr-4 py-3 bg-gray-50 border rounded-xl focus:ring-2 focus:ring-emerald-500 focus:bg-white outline-none transition-all font-semibold" onChange={(e) => setSearchTerm(e.target.value)} />
        </div>
      </div>
        ...
        <table className="w-full text-sm text-left">
          <thead className="bg-gray-50 border-b text-gray-500 font-bold uppercase text-[10px] tracking-widest">
            <tr>
              ...
              {!isAshaMode && <th className="px-6 py-4">Allocation</th>}
          ...
          <tbody className="divide-y divide-gray-100 font-semibold">
            {filteredData.length > 0 ? filteredData.map((b) => (
              <tr 
                key={b.id} 
                // 3. ROW CLICK TO OPEN DASHBOARD
                onClick={() => navigate(isAshaMode ? `/asha/beneficiary/${b.id}` : `/admin/beneficiary/${b.id}`)}
                className={`hover:bg-emerald-50/50 cursor-pointer transition-colors ${getAlertStatus(b.edd, b.assigned_asha_id)}`}
              >
                {!isAshaMode && (
                ...
                <td className="px-6 py-4">
                  ...
                  <div className="text-[10px] text-gray-400 font-mono flex items-center gap-2">
                    ...
                    {b.registration_source === 'asha_manual' && (
                        <span className="bg-blue-50 text-blue-500 px-1 rounded-[4px] text-[8px] font-black uppercase tracking-tighter">ASHA MANUAL</span>
                    )}
                  </div>
                </td>
                <td className="px-6 py-4">
                ...
                <td className="px-6 py-4 text-right">
                  <div className="flex justify-end gap-2">
                    {/* 4. ASHA DELETE MANUAL ENTRY ICON */}
                    {isAshaMode && b.registration_source === 'asha_manual' && (
                      <button 
                        onClick={(e) => {
                          e.stopPropagation(); // Prevent opening dashboard
                          handleDeleteManual(b.id);
                        }}
      ...
      {!isAshaMode && selectedIds.length > 0 && (
        <div className="fixed bottom-8 left-1/2 -translate-x-1/2 bg-gray-900 text-white px-6 py-4 rounded-2xl shadow-2xl flex items-center gap-6 z-50 animate-in slide-in-from-bottom-10">
          <div className="flex flex-col"><span className="text-xs text-gray-400 font-bold uppercase tracking-wider">Selected</span><span className="text-lg font-black text-emerald-400 leading-none">{selectedIds.length}</span></div>
          <div className="h-8 w-[1px] bg-gray-700" />
          <select className="bg-gray-800 border border-gray-700 rounded-xl text-sm px-4 py-2 outline-none focus:ring-2 focus:ring-emerald-500" value={bulkAshaId} onChange={(e) => setBulkAshaId(e.target.value)}>
            <option value="">Assign to ASHA...</option>
            {ashas.map(a => <option key={a.id} value={a.id}>{a.full_name} ({a.village})</option>)}
          </select>
          <button onClick={handleBulkAssign} className="bg-emerald-500 hover:bg-emerald-600 text-white px-6 py-2 rounded-xl text-sm font-bold transition-all flex items-center gap-2 active:scale-95">Update All</button>
          <button onClick={() => setSelectedIds([])} className="p-2 hover:bg-gray-800 rounded-full text-gray-400 transition-colors"><X size={20}/></button>
        </div>
      )}
          ...
          <form onSubmit={handleUpdate} className="bg-white rounded-3xl w-full max-w-2xl overflow-hidden shadow-2xl animate-in zoom-in-95 duration-200">
            <div className="p-6 border-b flex justify-between items-center bg-gray-50/50">
              <div>
                ...
                <p className="text-[10px] text-gray-400 font-mono uppercase">Role Access: {mode}</p>
              </div>
            ...
            <div className="p-8 space-y-6 max-h-[70vh] overflow-y-auto font-sans">
              <div className="grid grid-cols-2 gap-4">
                <div className="col-span-2 md:col-span-1">
                  ...
                  <input className="w-full mt-1 p-3 bg-gray-50 border rounded-xl focus:ring-2 focus:ring-emerald-500 outline-none font-semibold" value={editData.name} onChange={e => setEditData({...editData, name: e.target.value})} required />
                </div>
                <div className="col-span-2 md:col-span-1">
                  <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest">Govt ID</label>
              ...
              {!isAshaMode && (
                <div className="p-4 bg-emerald-50/50 rounded-2xl border border-emerald-100">
                    <label className="text-[10px] font-black text-emerald-700 uppercase tracking-widest flex items-center gap-1"><UserCheck size={12}/> Assigned ASHA Worker</label>
                    <select className="w-full mt-1 p-3 bg-white border border-emerald-200 rounded-xl font-bold text-emerald-900" value={editData.assigned_asha_id} onChange={e => setEditData({...editData, assigned_asha_id: e.target.value})}>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest">Village</label>
                ...
                <div>
                  <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest">Contact</label>
            ...
            <div className="p-6 bg-gray-50 border-t flex justify-end gap-3">
              ...
              <button type="submit" disabled={loading} className="px-8 py-2 bg-emerald-600 text-white rounded-xl font-bold shadow-lg shadow-emerald-100 hover:bg-emerald-700 flex items-center gap-2 active:scale-95 transition-all">
