...
const BeneficiaryDirectory = ({ mode = "admin" }) => {
  ...
  const getAlertStatus = (edd, ashaId, isHighRisk) => {
    ...
    if (diffDays <= 7 && diffDays >= 0)
      return "bg-rose-50 border-rose-200 animate-pulse-slow";
    return "";
  };
  ...
  const handleDeleteManual = async (id) => {
    ...
    const loadId = toast.loading(t("common.loading") || "Deleting...");
    try {
      await ashaService.deleteManualBeneficiary(id);
      toast.success(t("common.success") || "Record deleted successfully", {
        id: loadId,
      });
      fetchData();
    } catch (err) {
      toast.error(t("common.error") || "Failed to delete record", {
        id: loadId,
      });
    }
  };
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      const fileName = `Beneficiaries_${date}.csv`;
  ...
  const filteredData = data.filter((b) => {
    ...
    return (
      matchesSearch &&
      matchesStatus &&
      (selectedVillage === "all" ? true : b.village === selectedVillage)
    );
  });
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto space-y-6 pb-24">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <div
          className={`bg-white p-4 rounded-2xl border shadow-sm flex items-center gap-4 cursor-pointer transition-all ${
            filter === "all"
        ...
        >
          ...
          <div>
            <p className="text-[10px] font-bold text-gray-400 uppercase">
              {isAshaMode ? t("cases.my_cases") : t("admin.total_registry")}
            </p>
      ...
      <div className="bg-white p-6 rounded-2xl border border-gray-200 shadow-sm space-y-4">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          <h1 className="text-2xl font-bold text-gray-800">
            {isAshaMode ? t("cases.directory") : t("nav.beneficiaries")}
          </h1>
          <div className="flex flex-wrap items-center gap-3">
            ...
            <div className="flex bg-gray-100 p-1 rounded-xl border">
              ...
              >
                <ShieldAlert size={14} />{" "}
                {t("dashboard.high_risk").toUpperCase()}
              </button>
              <button
                onClick={() => setFilter("unassigned")}
                className={`flex items-center gap-2 px-4 py-1.5 rounded-lg text-xs font-bold transition-all ${
              ...
              >
                <UserCheck size={14} />{" "}
                {t("admin.pending_allocation").toUpperCase()}
              </button>
              <button
                onClick={() => setFilter("incomplete")}
                className={`flex items-center gap-2 px-4 py-1.5 rounded-lg text-xs font-bold transition-all ${
              ...
              >
                ...
                {t("cases.incomplete").toUpperCase()}
              </button>
            </div>
            {!isAshaMode && <button
              onClick={handleExport}
              className="flex items-center gap-2 px-4 py-2 bg-white border border-emerald-200 text-emerald-700 rounded-xl text-sm font-bold hover:bg-emerald-50 shadow-sm"
            >
      ...
      {!isAshaMode && selectedIds.length > 0 && (
        <div className="sticky top-0 z-20 bg-emerald-50 border border-emerald-200 rounded-2xl px-6 py-4 flex flex-col md:flex-row items-center justify-between gap-4 shadow-sm">
          {/* Left: Selected Count */}
          <div className="flex items-center gap-3 text-emerald-800 font-bold">
            <CheckSquare size={18} />
            <span>
              Selected: <span className="text-lg">{selectedIds.length}</span>
            </span>
          </div>

          {/* Right: ASHA Select + Update */}
          <div className="flex items-center gap-3">
            <select
              value={bulkAshaId}
              onChange={(e) => setBulkAshaId(e.target.value)}
              className="px-4 py-2 rounded-xl border bg-white font-semibold text-sm"
            >
        ...
        <table className="w-full text-sm text-left">
          <thead className="bg-gray-50 border-b text-gray-500 font-bold uppercase text-[10px] tracking-widest">
            <tr>
              ...
              {!isAshaMode && (
                <th className="px-6 py-4">{t("admin.workload")}</th>
              )}
              <th className="px-6 py-4 text-right">
                {t("cases.table_action")}
              </th>
            </tr>
          </thead>
              ...
              filteredData.map((b) => (
                <tr
                  ...
                  onClick={() =>
                    navigate(
                      isAshaMode
                        ? `/asha/beneficiary/${b.id}`
                        : `/admin/beneficiary/${b.id}`
                    )
                  }
                  className={`transition-colors cursor-pointer ${getAlertStatus(
                    b.edd,
                    b.assigned_asha_id,
                    b.is_high_risk
                  )}`}
                >
                  ...
                  <td className="px-6 py-4">
                    <div className="flex flex-col gap-1">
                      <div className="flex items-center gap-2">
                        <span
                          className={`font-bold ${
                            b.is_high_risk ? "text-red-900" : "text-gray-900"
                          }`}
                        >
                          {b.name}
                        </span>
                        {b.is_high_risk && (
                          <span className="bg-red-600 text-white text-[8px] font-black px-1.5 py-0.5 rounded uppercase tracking-tighter">
                            {t("forms.high_risk_alert")}
                          </span>
                        )}
                      </div>

                      {/* TAG: ASHA_MANUAL BELOW THE NAME */}
                      {b.registration_source === "asha_manual" && (
                        <div className="flex">
                          <span className="bg-amber-100 text-amber-700 text-[9px] font-black px-2 py-0.5 rounded-md border border-amber-200 uppercase tracking-wider">
                            ASHA_MANUAL
                          </span>
                        </div>
                      )}
                    ...
                    <td className="px-6 py-4">
                      <div
                        className={`flex items-center gap-2 font-bold ${
                          b.asha_name ? "text-emerald-700" : "text-gray-300"
                        }`}
                      >
                        <UserCheck size={14} />{" "}
                        {b.asha_name || (
                          <span className="font-normal italic text-xs uppercase tracking-tighter">
                            Needs ASHA
                          </span>
                        )}
                      </div>
                  ...
                  <td className="px-6 py-4 text-right">
                    <div className="flex justify-end gap-2">
                      {/* DELETE BUTTON: POSITIONED RIGHT BEFORE EDIT BUTTON */}
                      {isAshaMode &&
                        b.registration_source === "asha_manual" && (
                          <button
                            onClick={(e) => {
                            ...
                            title={t("common.delete") || "Delete Manual Record"}
                          >
                            <Trash2 size={16} />
                          </button>
                        )}
  ...
  <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
    <form
      ...
      className="bg-white rounded-3xl w-full max-w-2xl shadow-xl overflow-hidden animate-in zoom-in-95"
    >
      {/* Header */}
      <div className="p-6 flex justify-between items-center border-b bg-gray-50">
        <h3 className="text-xl font-extrabold text-gray-900">{t("common.edit")}</h3>
        <button
          type="button"
      ...
      <div className="p-8 space-y-6 max-h-[75vh] overflow-y-auto">
        ...
        <div
          onClick={() =>
            setEditData({ ...editData, is_high_risk: !editData.is_high_risk })
          }
          className={`p-4 rounded-2xl border-2 cursor-pointer transition-all flex items-center justify-between shadow-sm ${
            editData.is_high_risk
        ...
        >
          <div className="flex items-center gap-3">
            <AlertCircle
              className={
                editData.is_high_risk ? "animate-pulse text-red-600" : "text-gray-300"
              }
              size={24}
            />
            <div>
              ...
              <p className="text-xs opacity-80">{t("forms.mark_priority")}</p>
            </div>
          </div>
          <div
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            ...
            <input
              className="w-full mt-1 p-3 border rounded-xl bg-gray-50 focus:ring-2 focus:ring-emerald-400 outline-none transition"
              value={editData.name}
          ...
          <div>
            ...
            <input
              className="w-full mt-1 p-3 border rounded-xl bg-gray-50 font-mono focus:ring-2 focus:ring-emerald-400 outline-none transition"
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {["state", "district", "block"].map((field) => (
            <div key={field}>
              <label className="text-[10px] font-bold text-gray-500 uppercase">
                {t(`forms.${field}`)}
              </label>
        ...
        {/* Village & Contact */}
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {["village", "contact_number"].map((field) => (
            <div key={field}>
              <label className="text-[10px] font-bold text-gray-500 uppercase">
                {t(`forms.${field}`)}
              </label>
        ...
        {/* Admin-only Assigned ASHA */}
        {mode === "admin" && (
          <div>
            <label className="text-[10px] font-bold text-gray-500 uppercase">
              {t("forms.assigned_asha")}
            </label>
            <select
              className="w-full mt-1 p-3 border rounded-xl bg-gray-50 font-semibold focus:ring-2 focus:ring-emerald-400 outline-none transition"
              value={editData.assigned_asha_id || ""}
              onChange={(e) =>
                setEditData({ ...editData, assigned_asha_id: e.target.value })
              }
            >
              <option value="">Select ASHA</option>
              {ashas.map((a) => (
                <option key={a.id} value={a.id}>
                  {a.full_name} - {a.village}
                </option>
              ))}
            </select>
          </div>
      ...
      <div className="p-6 flex justify-end gap-3 border-t bg-gray-50">
        ...
        <button
          type="submit"
          disabled={loading}
          className="px-8 py-2 bg-emerald-600 text-white rounded-xl font-bold flex items-center gap-2 hover:bg-emerald-700 transition"
        >
